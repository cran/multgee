...
We describe the arguments of the core GEE functions (\code{nomLORgee}, \code{ordLORgee}) in Section [3](#Description1) while the utility functions (\code{waldts}, \code{confint}, \code{intrinsic.pars}, \code{vcov}, \code{matrixLOR}, \code{gee_criteria}) are described in Section [4](#Description2).
...
For completeness' sake, we also present computational related arguments (\code{LORem}, \code{add}, \code{bstart}, \code{LORgee_control}, \code{ipfp.control}, \code{IM}).
...
For completeness, the function \code{gee_criteria} reports commonly used criteria for variable selection and for selecting the "working" association structure. The Quasi Information Criterion (QIC) proposed by @pan2001b, the Correlation Information Criterion (CIC) proposed by @hin2009 and the Rotnitzky and Jewell's Criterion (RJC) proposed by @rotnitzky1990 are used for selecting the best association structure. The QICu criterion [@pan2001b] is used for selecting the best subset of covariates. When choosing among GEE models with different association structures but with the same subset of covariates, the model with the smallest value of QIC, CIC or RJC should be preferred. However, note that these criteria might not be very useful for multinomial GEE models with local odds ratios structures and the user should prefer the empirical rule suggested in @Touloumis2012 and implemented in \code{intrinsic.pars}. When choosing between two or more GEE models with different number of covariates that are not nested, the model with the smallest QICu value should be preferred. The function  \code{gee_criteria} can be used to obtain the values for the above criteria for one or more models of the class \code{LORgee}.
...
Alternatively, we can compare these two models using commonly used criteria:
```{r}
