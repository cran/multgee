derivacl <- function(fitprob, ncategoriesm1, X_mat) {
    n1 <- nrow(X_mat)
    n2 <- ncol(X_mat)
    z1 <- n1/ncategoriesm1
    ...
    for (i in seq_len(nrow(fitprob1))) {
        mat2 <- matrix(1 - cumprob1[i, ], ncategoriesm1, ncategoriesm1, 
            TRUE)
        mat3 <- 1 - mat2
    ...
    if (n2 > ncategoriesm1) {
        ...
        if (length(dummy2) == 1) {
            ...
            for (i in seq_len(nrow(fitprob1))) {
                dummy <- ((i - 1) * ncategoriesm1 + 1):(i * ncategoriesm1)
                mat3 <- .colSums(mat2[dummy, ], ncategoriesm1, dummy3, 
                  FALSE)
                mat3 <- X_mat[dummy, dummy2] - matrix(mat3, ncategoriesm1, 
                  dummy3, TRUE)
